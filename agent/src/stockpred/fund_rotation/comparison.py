...
def build_comparison(
    ...
    *,
    ...
    for variant in inputs:
        ...
        comparison_quality = variant.decision_quality
        if comparison_quality == "RESEARCH_ONLY_UNVERIFIED_UNIVERSE":
            # Universe qualification is a separate axis from decision quality.
            # Keep exploratory runs comparable, but visibly degrade their rank.
            comparison_quality = "DEGRADED"
        if comparison_quality not in ("VALID", "DEGRADED", "INVALID"):
            excluded.append(
                {"variant_key": key, "reason": EXCLUDED_TECHNICAL_FAILURE}
            )
            continue
        ...
        ranked_entries.append(
            {
                ...
                "quality_status": comparison_quality,
