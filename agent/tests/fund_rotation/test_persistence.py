...
class TestAtomicWrite:
    ...
    def test_no_tmp_left(self, tmp_path):
        ...
        assert not list(tmp_path.glob(".test.json.*.tmp"))

    def test_retries_permission_error(self, tmp_path, monkeypatch):
        p = tmp_path / "test.json"
        ...
        monkeypatch.setattr(
            "src.stockpred.fund_rotation.persistence.os.replace",
            flaky_replace,
        )
        monkeypatch.setattr(
            "src.stockpred.fund_rotation.persistence.time.sleep",
            lambda _: None,
        )
    ...
    def test_raises_after_permission_error_retries(
        self,
        tmp_path,
        monkeypatch,
    ):
    ...
    def test_uses_unique_temp_file(self, tmp_path, monkeypatch):
        p = tmp_path / "test.json"
        observed_sources = []
        real_replace = os.replace

        def capture_replace(src, dst):
            observed_sources.append(Path(src))
            return real_replace(src, dst)
        ...
        assert all(tmp.parent == p.parent for tmp in observed_sources)
