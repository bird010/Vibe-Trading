...
def _run(session, *, requirements=None, token=None, runner_run_id=None,
         simulation_start_date=None, run_id=None, dim_fund=None, snapshot=None):
...
def test_no_pit_resolver_uses_static_snapshot_universe_not_raw_candidates():
    observed = {}
    snapshot = PinnedFundDataSnapshot(
        ...
        dim_version=1,
        universe_codes=("A", "B"),
        historical_candidate_codes=("A", "B", "NON_ETF_LOF"),
        trading_dates=tuple(MARKET_DATES),
        fingerprint="test-fingerprint-with-raw-candidates",
    )

    def inspect_universe(context):
        observed["codes"] = {
            instrument.ts_code
            for instrument in context.data_view.eligible_universe()
        }
        return _decision(context.signal_date, DecisionKind.HOLD_TARGETS, seq=1)

    result, _ = _run(
        FakeSession({"20240112": inspect_universe}),
        dim_fund=pd.DataFrame([
            {"ts_code": "A", "name": "ETF Alpha", "list_date": "20200101"},
            {"ts_code": "B", "name": "ETF Beta", "list_date": "20200101"},
            {"ts_code": "NON_ETF_LOF", "name": "原油 LOF", "list_date": "20200101"},
        ]),
        snapshot=snapshot,
    )

    assert result.status is SubRunStatus.SUCCEEDED
    assert observed["codes"] == {"A", "B"}
