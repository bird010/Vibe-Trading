...
Covers clustering determinism/label normalization, pairwise exclusion, and
gate PASS/WARN/REJECT diagnostics with stable codes/thresholds/actuals/
affected codes. Portfolio construction remains the strategy session's
responsibility, not the gate module's.
"""
...
def test_gate_module_does_not_create_portfolio_decisions():
    import backtest.fund_rotation.strategies.correlation_representative.gates as gates

    assert not hasattr(gates, "cluster_quality_rejection_decision")
