...
def test_matches_pre_fix_golden():
    """Current pipeline output must match the frozen native-semantics golden.
...
def test_native_golden_has_no_legacy_capacity_zero_residual_events():
    expected = json.loads(GOLDEN_PATH.read_text(encoding="utf-8"))
    assert not [
        row for row in expected["orders"]
        if row.get("direction") == "SELL"
        and row.get("reason") == "capacity_zero"
        and row.get("attempt_status") == "BLOCKED"
        and row.get("attempt_filled") == 0
        and row.get("remaining") == 0
    ]
    assert not [
        row for row in expected["trade_events"]
        if row.get("action") == "SELL"
        and row.get("reason") == "capacity_zero"
        and row.get("status") == "BLOCKED"
        and row.get("requested") == 0
        and row.get("filled") == 0
        and row.get("unfilled") == 0
        and row.get("remaining") == 0
    ]
