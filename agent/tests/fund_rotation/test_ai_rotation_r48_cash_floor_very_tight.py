"""Focused tests for R48 very-tight cash-floor carry."""

import pytest

from backtest.fund_rotation.strategies.ai_rotation_r39_incumbent_carry.strategy import apply_incumbent_carry
from backtest.fund_rotation.strategies.ai_rotation_r45_cash_floor_carry.strategy_r48_cash_floor_very_tight import CASH_FLOOR, DESCRIPTOR, AiRotationR48CashFloorVeryTightStrategy, apply_cash_floor_carry


def test_cash_floor_reduces_extreme_carry_to_one_twenty_fourth():
...
def test_registered_identity_and_pipeline_are_r48_specific():
    strategy = AiRotationR48CashFloorVeryTightStrategy()
    pipeline = strategy.describe_decision_pipeline(strategy.config_model())
    assert DESCRIPTOR.id == "ai_rotation_r48_cash_floor_very_tight"
    assert "1/24" in str(pipeline)
