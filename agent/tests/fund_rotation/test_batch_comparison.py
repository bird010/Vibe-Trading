...
class TestEligibility:
    def test_research_only_unverified_universe_is_comparable_with_degraded_quality(self):
        outcome = build_comparison(
            [
                VariantComparisonInput(
                    variant_key="s@research",
                    strategy_id="s",
                    run_id="r-research",
                    status="SUCCEEDED",
        ...
        assert "s@research" in outcome.equity_frame.columns
        assert outcome.ranking[0]["quality_status"] == "DEGRADED"
