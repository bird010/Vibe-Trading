"""Focused tests for R52 femto cash-floor carry."""

import pytest

from backtest.fund_rotation.strategies.ai_rotation_r39_incumbent_carry.strategy import apply_incumbent_carry
from backtest.fund_rotation.strategies.ai_rotation_r45_cash_floor_carry.strategy_r52_cash_floor_femto import CASH_FLOOR, DESCRIPTOR, AiRotationR52CashFloorFemtoStrategy, apply_cash_floor_carry


def test_cash_floor_reduces_extreme_carry_to_one_three_hundred_eighty_fourth():
...
def test_registered_identity_and_pipeline_are_r52_specific():
    strategy = AiRotationR52CashFloorFemtoStrategy()
    pipeline = strategy.describe_decision_pipeline(strategy.config_model())
    assert DESCRIPTOR.id == "ai_rotation_r52_cash_floor_femto"
    assert "1/384" in str(pipeline)
