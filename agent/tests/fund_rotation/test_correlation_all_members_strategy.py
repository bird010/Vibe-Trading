...
class TestStrategyProtocol:
    ...
    def test_session_evaluate_produces_real_decision(self):
        ...
        scored_candidates = [
            candidate
            for candidate in diagnostics.decision_trace[-1]["candidates"]
            if candidate["score"] is not None
        ]
        assert scored_candidates
        assert all(
            candidate["score"]["model_id"] == "cluster_momentum"
            for candidate in scored_candidates
        )
        assert all(
            candidate["stages"]["cluster_representative"] is False
            for candidate in scored_candidates
        )
    ...
    def test_session_coverage_eligibility_uses_each_momentum_week(self, monkeypatch):
        ...
        def fake_scores(_self, _returns, _clusters, _window, **_kwargs):
            return {
                1: StrategyScore(
                    value=0.10,
                    eligible=True,
        ...
        monkeypatch.setattr(strategy_module.ClusterMomentumScoreModel, "score", fake_scores)
