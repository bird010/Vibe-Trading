...
def build_research_static_execution_rule_context(
    *,
    ...
    for code in codes:
        ...
        rule = _RESEARCH_STATIC_ETF_RULES.get(instrument_type or "")
        if rule is None:
            raise UnknownExecutionRule(
                "UNKNOWN_EXECUTION_RULE: unsupported research-static "
                f"instrument {code}: instrument_type={instrument_type!r}"
            )
        ...
        records.append(
            {
                ...
                "price_limit_rule": rule["price_limit_rule"],
                "short_allowed": False,
                "currency": "CNY",
    ...
    if set(instruments) != set(codes):
        missing = sorted(set(codes) - set(instruments))
        extra = sorted(set(instruments) - set(codes))
        raise UnknownExecutionRule(
            "RESEARCH_STATIC_UNIVERSE_MISMATCH: "
...
def _resolve_research_instrument_type(
    ...
    # and has no asset_class/instrument_type columns.  This compatibility is
    # local to Research Static; the formal PIT mapper remains strict about
    # fund_type == ETF.
    legacy_types = {
        ...
        "货币市场型": "money_market_etf",
        # Audited pinned dim_fund v56: every ETF with fund_type=其他 is a
        # commodity/gold ETF (518xxx and commodity-futures products).
        "其他": "commodity_etf",
    }
    return legacy_types.get(fund_type or "")


_RESEARCH_STATIC_ETF_RULES: dict[str, dict[str, object]] = {
    "domestic_equity_etf": {
        "settlement": "T+1",
        "lot_size": 100,
        "tick_size": 0.001,
