...
def apply_hysteresis(
    ...
    *,
    ...
    ranking_order: Sequence[Any] | None = None,
) -> HysteresisResult:
    if policy.top_n <= 0:
    ...
    fallback_order = [
        ...
        for cluster_id, _score in sorted(
            scores.items(), key=lambda item: (-_finite_or_worst(item[1]), str(item[0]))
        )
    ]
    ordered_ids = list(ranking_order or fallback_order)
    ordered_ids.extend(cluster_id for cluster_id in fallback_order if cluster_id not in ordered_ids)
    ranked = [
        cluster_id
        for cluster_id in ordered_ids
        if (
            cluster_id in scores
            and math.isfinite(float(scores[cluster_id]))
            and float(scores[cluster_id]) > policy.minimum_entry_score
    ...
    rank_order = {cluster_id: rank for rank, cluster_id in enumerate(ranked)}
    retained = sorted(retained, key=lambda cluster_id: rank_order.get(cluster_id, len(rank_order)))
    ...
    if slots > 0:
        ...
        if slots > 0:
            fallback_holdings = sorted(
                ...
                key=lambda cluster_id: rank_order.get(cluster_id, len(rank_order)),
            )
    ...
    selected = sorted(selected, key=lambda cluster_id: rank_order.get(cluster_id, len(rank_order)))
    next_holdings = {
