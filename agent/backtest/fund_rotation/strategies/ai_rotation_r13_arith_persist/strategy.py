"""Round 13 challenger: rank clusters by persistent arithmetic momentum.
...
positive, then ranking eligible clusters by their equal-weight arithmetic
mean.
"""
...
DESCRIPTOR = FundRotationStrategyDescriptor(
    id="ai_rotation_r13_arith_persist",
    name="持续算术动量相关性代表ETF",
    description=(
        "复用相关性聚类代表ETF流程，要求当前及向后错一周的四周簇动量"
        "均严格为正，并按两者动量的等权算术平均排名。"
    ),
...
_SCORE_MODEL_ID = "arithmetic_persistent_cluster_momentum"
_SCORE_MODEL_LABEL = "Arithmetic Persistent Cluster Momentum"
...
def arithmetic_persistent_score(
    ...
    return StrategyScore(
        ...
        components={
            ...
            "arithmetic_persistent_momentum": arithmetic,
            "arithmetic_delta": arithmetic_delta,
        },
    )


def compute_arith_persist_scores(
    ...
    delta_values: dict[int, float | None]
    ...
    if len(weekly_returns) < momentum_window + 1:
        ...
        delta_values = {cluster_id: None for cluster_id in cluster_ids}
    else:
        ...
        delta_values = {
            cluster_id: (
                current_values[cluster_id] - lagged_values[cluster_id]
                if current_values.get(cluster_id) is not None
                and lagged_values.get(cluster_id) is not None
                else None
            )
    ...
    scores = {
        cluster_id: arithmetic_persistent_score(
    ...
    return scores, current_values, lagged_values, delta_values, arithmetic_values


class AiRotationR13ArithPersistSession(CorrelationRepresentativeSession):
    """Isolated session with round 13 persistent-geometry ranking."""
    ...
    def evaluate(self, context: StrategyDecisionContext) -> TargetWeightDecision:
        ...
        scores, current_raw, lagged_raw, delta_raw, arithmetic_raw = compute_arith_persist_scores(
            momentum_window,
            self._clusters,
        ...
        delta_values, delta_unavailable = _momentum_diagnostics(
            {
                cluster_id: value if value is not None else math.nan
                for cluster_id, value in delta_raw.items()
            }
        )
        arithmetic_values, arithmetic_unavailable = _momentum_diagnostics(
            {
                cluster_id: value if value is not None else math.nan
                for cluster_id, value in arithmetic_raw.items()
            }
        )
        unavailable = sorted(
            set(current_unavailable)
            | set(lagged_unavailable)
            | set(delta_unavailable)
            | set(arithmetic_unavailable)
        )
        ...
        decision = TargetWeightDecision(
            ...
            diagnostics={
                ...
                "arithmetic_delta": delta_values,
                "arithmetic_delta_unavailable_clusters": delta_unavailable,
                "arithmetic_momentum": arithmetic_values,
                "arithmetic_momentum_unavailable_clusters": arithmetic_unavailable,
...
class AiRotationR13ArithPersistStrategy:
    """Complete round 13 strategy plug-in."""
    ...
    def describe_decision_pipeline(self, config: BaseModel) -> dict[str, object]:
        ...
        pipeline["selection_rule"] = (
            ...
            "one-week-lagged momentum, ranked by "
    ...
    def create_session(
        ...
        config: BaseModel,
    ) -> AiRotationR13ArithPersistSession:
        return AiRotationR13ArithPersistSession(config)  # type: ignore[arg-type]
