"""Round 20 challenger: front-load the three fixed portfolio slots."""
...
DESCRIPTOR = FundRotationStrategyDescriptor(
    id="ai_rotation_r20_rank_frontload",
    name="排名前置持续动量相关性代表ETF",
    description=(
        "复用持续几何动量相关性代表ETF流程，仅对前三个固定排名槽使用"
        "1/2、1/3、1/6 权重；空槽保留为现金且不补位。"
    ),
...
SLOT_WEIGHTS = (1 / 2, 1 / 3, 1 / 6)


def build_rank_frontload_slot_weights(
    ranked_cluster_ids: list[int],
    representatives: Mapping[int, str | None],
) -> tuple[dict[str, float], list[int], list[int], float]:
    """Map the first three fixed slots without backfilling or reweighting."""
    weights: dict[str, float] = {}
    filled: list[int] = []
    vacant: list[int] = []
    for slot, cluster_id in enumerate(ranked_cluster_ids[: len(SLOT_WEIGHTS)]):
        representative = representatives.get(cluster_id)
        if representative:
            weights[representative] = weights.get(representative, 0.0) + SLOT_WEIGHTS[slot]
...
class AiRotationR20RankFrontloadSession(AiRotationR11PersistGeomSession):
    """R11 causal session with fixed rank-frontloaded slot weights."""
    ...
    def evaluate(self, context: StrategyDecisionContext) -> TargetWeightDecision:
        ...
        weights, filled, vacant, cash_weight = build_rank_frontload_slot_weights(
...
class AiRotationR20RankFrontloadStrategy(AiRotationR11PersistGeomStrategy):
    """Complete round 20 strategy plug-in."""
    ...
    def describe_decision_pipeline(self, config: BaseModel) -> dict[str, object]:
        ...
        pipeline["selection_rule"] = (
            "Top 3 persistent geometric-momentum clusters with fixed rank slot weights "
            "1/2, 1/3, 1/6; vacant slots are not backfilled and remain cash"
        )
        return pipeline
    ...
    def create_session(self, initialization: StrategyInitializationContext, config: BaseModel) -> AiRotationR20RankFrontloadSession:
        return AiRotationR20RankFrontloadSession(config)  # type: ignore[arg-type]
