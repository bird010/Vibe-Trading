"""Round 53: preserve an atto cash floor after R39 carry."""
...
DESCRIPTOR = FundRotationStrategyDescriptor(
    id="ai_rotation_r53_cash_floor_atto",
    name="承接释放权重保留七百六十八分之一现金持续几何动量相关性代表ETF",
    description=("完全沿用 R39；仅当 carry 后现金低于 1/768 时按原 carry 比例削减 carry，使现金回到 1/768，其余情况逐值沿用 R39。"),
...
class AiRotationR53CashFloorAttoSession(AiRotationR39IncumbentCarrySession):
    def evaluate(self, context: StrategyDecisionContext) -> TargetWeightDecision:
        ...
        diagnostics.update({"staged_reentry_codes": sorted(staged), "incumbent_carry_codes": sorted(incumbents), "cash_floor": CASH_FLOOR, "cash_floor_rule": "preserve_one_seven_hundred_sixty_eighth_cash_after_r39_carry_only_when_floor_breached"})
...
class AiRotationR53CashFloorAttoStrategy(AiRotationR39IncumbentCarryStrategy):
    descriptor = DESCRIPTOR

    def describe_decision_pipeline(self, config: BaseModel) -> dict[str, object]:
        pipeline = super().describe_decision_pipeline(config)
        pipeline["selection_rule"] += "; preserve a fixed 1/768 cash floor after R39 carry only when breached"
        return pipeline

    def resolve_requirements(self, config: BaseModel) -> StrategyDataRequirements:
        return super().resolve_requirements(config)

    def create_session(self, initialization: StrategyInitializationContext, config: BaseModel) -> AiRotationR53CashFloorAttoSession:
        del initialization
        return AiRotationR53CashFloorAttoSession(config)  # type: ignore[arg-type]
