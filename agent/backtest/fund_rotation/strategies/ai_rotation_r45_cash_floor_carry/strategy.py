"""Round 45: preserve a fixed cash floor after R39 carry."""
...
DESCRIPTOR = FundRotationStrategyDescriptor(
    id="ai_rotation_r45_cash_floor_carry",
    name="承接释放权重保留六分之一现金持续几何动量相关性代表ETF",
    description=(
        "完全沿用 R39；仅当 carry 后现金低于 1/6 时按原 carry 比例削减 carry，"
        "使现金回到 1/6，其余情况逐值沿用 R39。"
    ),
...
def apply_cash_floor_carry(
    ...
    baseline_targets, _, baseline_staged = _r34_baseline(previous_weights, staged_target_weights)
    carry_by_code = {
        code: r39_targets.get(code, 0.0) - baseline_targets.get(code, 0.0)
    ...
    if not math.isfinite(adjusted_total):
        return r39_targets, r39_cash, r39_staged, r39_incumbents, bool(r39_incumbents)
    return adjusted, CASH_FLOOR, r39_staged, r39_incumbents, True


class AiRotationR45CashFloorCarrySession(AiRotationR39IncumbentCarrySession):
    def evaluate(self, context: StrategyDecisionContext) -> TargetWeightDecision:
        ...
        target_weights, cash_weight, staged, incumbents, carry_applied = apply_cash_floor_carry(
        ...
        diagnostics.update({
            ...
            "cash_floor_rule": "preserve_one_sixth_cash_after_r39_carry_only_when_floor_breached",
        })
        decision = replace(
            decision,
...
class AiRotationR45CashFloorCarryStrategy(AiRotationR39IncumbentCarryStrategy):
    descriptor = DESCRIPTOR

    def describe_decision_pipeline(self, config: BaseModel) -> dict[str, object]:
        pipeline = super().describe_decision_pipeline(config)
        pipeline["selection_rule"] += "; preserve a fixed 1/6 cash floor after R39 carry only when breached"
        return pipeline

    def resolve_requirements(self, config: BaseModel) -> StrategyDataRequirements:
        return super().resolve_requirements(config)

    def create_session(self, initialization: StrategyInitializationContext, config: BaseModel) -> AiRotationR45CashFloorCarrySession:
        del initialization
        return AiRotationR45CashFloorCarrySession(config)  # type: ignore[arg-type]
