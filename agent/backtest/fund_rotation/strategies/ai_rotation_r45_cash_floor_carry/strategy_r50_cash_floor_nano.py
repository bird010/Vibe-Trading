"""Round 50: preserve a nano cash floor after R39 carry."""
...
DESCRIPTOR = FundRotationStrategyDescriptor(
    id="ai_rotation_r50_cash_floor_nano",
    name="承接释放权重保留九十六分之一现金持续几何动量相关性代表ETF",
    description=("完全沿用 R39；仅当 carry 后现金低于 1/96 时按原 carry 比例削减 carry，使现金回到 1/96，其余情况逐值沿用 R39。"),
...
class AiRotationR50CashFloorNanoSession(AiRotationR39IncumbentCarrySession):
    def evaluate(self, context: StrategyDecisionContext) -> TargetWeightDecision:
        ...
        diagnostics.update({"staged_reentry_codes": sorted(staged), "incumbent_carry_codes": sorted(incumbents), "cash_floor": CASH_FLOOR, "cash_floor_rule": "preserve_one_ninety_sixth_cash_after_r39_carry_only_when_floor_breached"})
...
class AiRotationR50CashFloorNanoStrategy(AiRotationR39IncumbentCarryStrategy):
    descriptor = DESCRIPTOR

    def describe_decision_pipeline(self, config: BaseModel) -> dict[str, object]:
        pipeline = super().describe_decision_pipeline(config)
        pipeline["selection_rule"] += "; preserve a fixed 1/96 cash floor after R39 carry only when breached"
        return pipeline

    def resolve_requirements(self, config: BaseModel) -> StrategyDataRequirements:
        return super().resolve_requirements(config)

    def create_session(self, initialization: StrategyInitializationContext, config: BaseModel) -> AiRotationR50CashFloorNanoSession:
        del initialization
        return AiRotationR50CashFloorNanoSession(config)  # type: ignore[arg-type]
