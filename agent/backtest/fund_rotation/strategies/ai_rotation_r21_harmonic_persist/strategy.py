"""Round 21 challenger: rank persistent clusters by harmonic growth momentum."""
...
DESCRIPTOR = FundRotationStrategyDescriptor(
    id="ai_rotation_r21_harmonic_persist",
    name="持续调和动量相关性代表ETF",
    description=(
        "复用相关性聚类代表ETF流程，要求当前及向后错一周的四周簇动量"
        "均严格为正，并按两者增长因子的调和均值排名。"
    ),
    interface_version="1.0", supported_universe=("etf",), deterministic=True,
)
_SCORE_MODEL_ID = "persistent_harmonic_cluster_momentum"
_SCORE_MODEL_LABEL = "Persistent Harmonic Cluster Momentum"
...
def harmonic_persistent_score(current_momentum, lagged_momentum) -> StrategyScore:
    """Apply the Champion's strict-positive gate and harmonic growth score."""
    ...
    harmonic = None
    if eligible:
        denominator = 1.0 / (1.0 + current) + 1.0 / (1.0 + lagged)
        candidate = 2.0 / denominator - 1.0 if denominator > 0 else math.nan
        harmonic = candidate if math.isfinite(candidate) else None
        eligible = harmonic is not None
    return StrategyScore(
        value=harmonic, eligible=eligible, subject_id=None,
        display_label="持续调和簇动量", model_label=_SCORE_MODEL_LABEL,
        ...
        components={"current_momentum": current, "lagged_momentum": lagged,
                    "harmonic_persistent_momentum": harmonic},
    )


def compute_harmonic_persist_scores(
    ...
    """Compute causal current/lagged windows and harmonic scores per epoch."""
    ...
    if len(weekly_returns) < momentum_window + 1:
        ...
        current_values = {cid: score.value for cid, score in current_scores.items()}
        lagged_values = {cid: score.value for cid, score in lagged_scores.items()}
    scores = {cid: harmonic_persistent_score(current_values.get(cid), lagged_values.get(cid))
              for cid in cluster_ids}
    return scores, current_values, lagged_values, {cid: score.value for cid, score in scores.items()}


class AiRotationR21HarmonicPersistSession(CorrelationRepresentativeSession):
    """Isolated session retaining the Champion path with a new ranking score."""
    ...
    def evaluate(self, context: StrategyDecisionContext) -> TargetWeightDecision:
        ...
        scores, current_raw, lagged_raw, harmonic_raw = compute_harmonic_persist_scores(
        ...
        harmonic_values, harmonic_unavailable = _momentum_diagnostics({cid: v if v is not None else math.nan for cid, v in harmonic_raw.items()})
        unavailable = sorted(set(current_unavailable) | set(lagged_unavailable) | set(harmonic_unavailable))
        ...
        decision = TargetWeightDecision(
            ...
            diagnostics={
                ...
                "harmonic_momentum": harmonic_values,
                "harmonic_momentum_unavailable_clusters": harmonic_unavailable,
...
class AiRotationR21HarmonicPersistStrategy:
    ...
    def describe_decision_pipeline(self, config: BaseModel) -> dict[str, object]:
        ...
        pipeline["selection_rule"] = (f"Top {config.top_n} clusters with strictly positive current and "
                                      "one-week-lagged momentum, ranked by their harmonic growth mean")
        return pipeline
    ...
    def create_session(self, initialization: StrategyInitializationContext, config: BaseModel):
        return AiRotationR21HarmonicPersistSession(config)  # type: ignore[arg-type]
